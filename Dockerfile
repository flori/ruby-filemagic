...
RUN curl -L https://download.sourceforge.net/mingw/Other/UserContributed/regex/mingw-regex-$GNURX_VERSION/mingw-libgnurx-$GNURX_VERSION-src.tar.gz | \
    tar xzvf - && \
    cd $GNURX_SOURCE && \
...
RUN mkdir -p /root/.rake-compiler
ENV CROSS_RUBIES 3.4.5
